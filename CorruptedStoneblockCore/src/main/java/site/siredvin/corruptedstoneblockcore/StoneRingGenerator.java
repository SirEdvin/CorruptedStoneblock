...
public final class StoneRingGenerator extends ChunkGenerator {
    ...
    public record Ring(int width, BlockState block) {
        ...
        public static final Codec<Ring> CODEC = RecordCodecBuilder.create(i -> i.group(
                ...
                BlockState.CODEC.fieldOf("block").forGetter(Ring::block)).apply(i, Ring::new));
    }
    ...
    public static final Codec<StoneRingGenerator> CODEC = RecordCodecBuilder.create(i -> i.group(
            ...
            BlockState.CODEC.fieldOf("outer_block").forGetter(g -> g.outerBlock)).apply(i, StoneRingGenerator::new));
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                for (int y = low; y < high; y++) {
                    BlockState block = (y == minY || y == minY + height - 1) ? Blocks.BEDROCK.defaultBlockState() :
                            material;
    ...
    @Override
    public void buildSurface(WorldGenRegion level, StructureManager structures, RandomState random,
                             ChunkAccess chunk) {}
    ...
    public void addDebugScreenInfo(List<String> info, RandomState random, BlockPos pos) {
        info.add("Stone rings: center=" + centerX + "," + centerZ + " material=" +
                columnMaterial(pos.getX(), pos.getZ()));
    }
}
